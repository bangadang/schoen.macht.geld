 ...
 * @returns {JSX.Element | null} The rendered component, or null if no stocks are available.
 */
export default function StockChartClient() {
  ...
  const chartData = activeStock.history.map(h => ({
      ...h,
      timestamp: new Date(h.timestamp).toLocaleTimeString('de-DE', { hour: '2-digit', minute: '2-digit' }),
  }));

  const isPositive = activeStock.percentChange >= 0;
          ...
          <div className="flex items-start justify-between">
            <div className="flex items-center gap-6">
                <div className="relative w-32 h-32 rounded-full overflow-hidden border-4 border-gray-700">
                    <Image src={activeStock.photoUrl} alt={activeStock.nickname} layout="fill" objectFit="cover" />
                </div>
                <div>
                    <h1 className="text-6xl font-bold">{activeStock.nickname}</h1>
            ...
            <div className="flex flex-col items-end">
                <span className={cn(
                    ...
                    {activeStock.currentValue.toFixed(2)} <span className="text-5xl">CHF</span>
                </span>
                <div className={cn(
                    ...
                    <span>
                        {isPositive ? '+' : ''}{activeStock.change.toFixed(2)} ({activeStock.percentChange.toFixed(2)}%)
                    </span>
                </div>
            </div>
          </div>
              ...
              <AreaChart data={chartData} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
                <defs>
                  <linearGradient id={`color-${isPositive}`} x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%" stopColor={isPositive ? "hsl(142.1 76.2% 36.3%)" : "hsl(0 72.2% 50.6%)"} stopOpacity={0.8}/>
                    <stop offset="95%" stopColor={isPositive ? "hsl(142.1 76.2% 36.3%)" : "hsl(0 72.2% 50.6%)"} stopOpacity={0}/>
                  </linearGradient>
                </defs>
                <CartesianGrid strokeDasharray="3 3" stroke="rgba(255, 255, 255, 0.1)" />
                <XAxis dataKey="timestamp" tick={{ fill: 'white' }} tickLine={{ stroke: 'white' }} />
                <YAxis domain={['dataMin - 1', 'dataMax + 1']} tick={{ fill: 'white' }} tickLine={{ stroke: 'white' }} />
                <Tooltip
                  contentStyle={{ 
