...
def format_record(record: dict[str, Any]) -> str:  # pyright: ignore[reportExplicitAny]
    """
    ...
    format_string = str(LOGURU_FORMAT)  # pyright: ignore[reportUnknownArgumentType]
    if record["extra"].get("payload") is not None:  # pyright: ignore[reportAny]
        record["extra"]["payload"] = pformat(
            record["extra"]["payload"],  # pyright: ignore[reportAny]
            indent=4,
            compact=True,
            width=88,
        )
...
def init_logging():
    ...
    2020-07-25 02:19:21.357 | INFO     | uvicorn.lifespan.on:startup:34 - Application...
